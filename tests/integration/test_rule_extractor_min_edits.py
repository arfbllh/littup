"""Integration: groups below RULE_EXTRACTOR_MIN_EDITS never call the LLM."""
...
async def _seeded_two_edits(test_session_factory):
    ...
    async with test_session_factory() as s:
        ...
        # Only 2 edits — below MIN_EDITS=3
        for i in range(2):
            await s.execute(
                text(
...
@pytest.mark.asyncio
async def test_min_edits_no_llm_call(
    _seeded_two_edits, test_session_factory, cleanup_drafts_and_jobs
):
    entry, registry = _seeded_two_edits
    from app.edits import rule_extractor as _re

    router = MagicMock()
    router.generate = AsyncMock()  # Should never be called
    ...
    assert result.groups_skipped_min_edits == 1
